...
addBtn.addEventListener('click', () => {
    ...
    let newText = {
        ...
        fontSize: fontSize,
        bold: function () {
            return boldTextInput.checked === true ? true : false
        },
        italic: function () {
            return italicTextInput.checked === true ? true : false
        },
        align:true,
        underline: false,
        isDragging: false
...
canvas.addEventListener('dblclick', (e) => {
    ...
    texts.forEach(textObject => {
        let textWidth = ctx.measureText(textObject.text).width
        let textHeight = parseInt(`${textObject.fontSize}px ${textObject.fontStyle}`, 10)
